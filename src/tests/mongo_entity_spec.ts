...
describe("entity", () => {
  ...
  describe("findOneCachedById", () => {
    ite(
      "returns an item even when it was not previously found, but it wont search twice",
      async (entity) => {
    ...
    ite(
      "can find and cache results of finding a single item",
      async (entity) => {
    ...
    ite(
      "clears the related cache when a document is updated",
    ...
    ite(
      "clears all caches when a document is updated and selector is unknown",
      async function (entity) {
        let foo = await entity.findOneCachedById("1");
        let bar = await entity.findOneCachedById("2");
    ...
    ite(
      "findOneCached returns null if entity does not exists",
      async (entity) => {
        // first we test if DB is called every time
  ...
  describe("filter", () => {
    ...
    it("throws error when no filter is specified", function () {
      ...
      assert.throws(
        () => entity.filter(obj, {}),
        /You need to specify the selector!/
      );
    });
    ...
    it("throws error when you mix include exclude", function () {
      ...
      assert.throws(
        () => entity.filter(obj, { a: 1, b: 0 }),
        /You cannot combine include and exclude!/
      );
    });
  });
  ...
  describe("findManyCached", () => {
    it("can find and cache results of finding multiple items", async () => {
      await withEntity(async (entity) => {
        await entity.collection.insertOne({ _id: "1" });
  ...
  describe("custom loaders", () => {
    it("throws error when selectorKeyFunction is not specified with updates", async () => {
      await withEntity((entity) => {
        assert.throws(
          () => entity.createLoader(() => {}, { clearOnUpdate: true }),
  ...
  describe("LruCacheWrapper", () => {
    it("maps lru-cache functions", function () {
      ...
      const { LruCacheWrapper } = proxyquire("../mongo_entity", {
        "lru-cache": { LRUCache: cacheStub }
      });
      const cacheWrapper = new LruCacheWrapper();

      cacheWrapper.clear();
      sinon.assert.calledOnce(cacheWrapper.cache.clear);
      cacheWrapper.get();
      ...
      sinon.assert.calledWith(cacheWrapper.cache.delete, "1");
